/* Descarga otro proyecto haciendo clic en este enlace https://github.com/alura-es-cursos/js-curso-2/tree/main
y ábrelo en Visual Studio Code. */

/* Cambia el contenido de la etiqueta h1 con document.querySelector 
  y asigna el siguiente texto: "Hora del Desafío".*/
let titulo = document.querySelector("h1");
titulo.innerText = "Hora del Desafío";

/* Crea una función que muestre en la consola el mensaje 
"El botón fue clicado" siempre que se presione el botón "Console". */
function clicConsole() {
  console.log("El botón fue clicado");
}

/* Crea una función que se ejecute cuando se haga clic en 
  el botón "prompt", preguntando el nombre de una ciudad del mundo. 
  Luego, muestra una alerta con el mensaje concatenando la respuesta 
  con el texto: "Estuve en {ciudad} y me acordé de ti". */
function clicPrompt() {
  console.log("Hola");
  let ciudad = prompt("Escriba el nombre de una ciudad del mundo");
  alert(`Estuve en ${ciudad} y me acordé de ti`);
}

/* Crea una función que muestre una alerta con el mensaje: "Yo amo JS" 
  siempre que se presione el botón "Alerta". */
function clicAlert() {
  alert("Yo amo JS");
}

/* Al hacer clic en el botón "suma", pide 2 números y muestra el 
   resultado de la suma en una alerta.*/
function clicSuma() {
  let num1 = parseInt(prompt("Ingrese un primer numero"));
  let num2 = parseInt(prompt("Ingrese un segundo numero"));
  alert(`La suma de ${num1} y ${num2} es: ${num1 + num2}`);
}
